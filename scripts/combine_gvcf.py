...
def main():
	...
	path_to_sample_map = parser.sample_map
	...
	hl.init(
		...
		log=os.path.join(os.getcwd(), 'log')		# This log location must be local
	)
...
def init_log(path_to_output):
	...
	log_location = os.path.join(os.path.dirname(os.path.realpath(__file__)), f"combine_gvcf_{dt_string}.log")
